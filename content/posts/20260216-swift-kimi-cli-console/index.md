...
这不是科幻场景。只需要约 300 行 Swift 代码，零第三方依赖，就能把 iPhone 变成功能完备的 Kimi CLI 控制台。这个项目的特点是代码量少，更多时间花在理解架构和拆解任务上。
...
Client                     Server
  ...
  │<──── event: TurnBegin ───┤
  │<──── event: StepBegin ───┤
  │<─ event: ContentPart ────┤  流式推送（思考过程）
  │<─ event: ContentPart ────┤
  │<─── event: ToolCall ─────┤  工具调用
  │<── request: Approval ────┤  请求审批
  ├──── response: approve ──>│
  │<─── event: ToolResult ───┤  工具结果
  │<─ event: ContentPart ────┤  最终回复
  │<── session_status: idle ─┤  轮次结束
```
...
└── Info.plist               # HTTP 权限配置
```

### 4.2 HTTP 权限配置

iOS 默认通过 ATS（App Transport Security）禁止 HTTP 明文传输，而 Kimi Web UI 用的是 HTTP，直接加载时 ATS 会拦截请求。

在 `Info.plist` 中添加以下配置来解除限制：

```xml
<key>NSAppTransportSecurity</key>
<dict>
    <!-- 允许所有 HTTP 明文请求（本地开发用） -->
    <key>NSAllowsArbitraryLoads</key>
    <true/>
</dict>
```

本地、局域网场景下合理，生产环境应改用 HTTPS。
...
「本地 Web 服务 + WebView」是通用模式：后端有 HTTP 接口就能跨平台复用，代价是性能、体验不如原生。对于 AI Agent 这种交互密度不高的场景，WebView 够用。

### 6.3 快速开始

**30 分钟部署清单**：

1. 启动服务（真机用 `--network`）：`kimi web --network`
2. 获取 Mac 局域网 IP：`ipconfig getifaddr en0`
3. 克隆 iOS 项目，用 Xcode 打开
...
- [FastAPI WebSocket 指南](https://fastapi.tiangolo.com/advanced/websocket/)：为其他 CLI 工具添加 Web UI
